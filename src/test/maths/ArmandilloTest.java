...
public class ArmandilloTest {
    @Parameters
    public static List<Integer> params() {return Arrays.asList(0, 1, 2);}
    ...
    public void testDiffusionEquation(){
        ...
        Armadillo.solveDiffusionEquation(width, height, depth, data, isGiven, Armadillo.Backend.ARMADILLO);
        ...
        Arrays.fill(data, 1, data.length - 1, 1);
        Armadillo.solveDiffusionEquation(width, height, depth, data, isGiven, Armadillo.Backend.CUPY);
        for (int i = 0; i < data.length; ++i)
        {
            double expected = 4.5 * 4.5 * 0.5 - 0.5 * (i - 4.5) * (i - 4.5);
            assertTrue("expected " + expected + "got data[" + i + "]=" + data[i], Math.abs(expected - data[i]) < 0.01);
